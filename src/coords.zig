...
pub fn main() !void {
    ...
    // transformations
    const model = linalg.Mat4.fromRotation(-55, linalg.Vec3.new(1, 0, 0));
    const view = linalg.Mat4.fromTranslate(linalg.Vec3.new(0, 0, -3));
    const projection = linalg.Mat4.perspective(45.0, @as(f32, @floatFromInt(window_width / window_height)), 0.1, 100.0);
